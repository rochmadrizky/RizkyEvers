...
const SearchModal = ({ membuka, menutup }) => {
  const [search, mengaturSearch] = useState("");
  const [opsiLengkap, mengaturOpsiLengkap] = useState([]);
  const [pilihOpsiIndex, mengaturPilihOpsiIndex] = useState(-1);
  ...
  const dapatkanDeskripsi = (halaman) => {
    return halamanInfo[halaman]?.deskripsi || "";
  };

  const pilihanLengkap = (opsi) => {
